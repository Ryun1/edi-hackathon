...
// Just submit the tx passed
export const submitDelegation = async (delegationCertificate) => {
  const result = await submitTx(delegationCertificate);
  return {tx: delegationCertificate, txHash: result.toString(), witness: "witness"};
};

// Just submit the tx passed
export const submitDRepRegistrationCertificate = async (dRepRegistrationCertificate) => {
  const result = submitTx(dRepRegistrationCertificate);
  return result;
};

// Just submit the tx passed
export const submitDRepRetirementCertificate = async (dRepRetirementCertificate) => {
  const result = submitTx(dRepRetirementCertificate);
  return result;
};

// Just submit the tx passed
export const submitVote = async (vote) => {
  const result = submitTx(vote);
  return result;
};

// Just submit the tx passed
export const submitGovernanceAction = async (governanceAction) => {
  const result = submitTx(governanceAction);
  return result;
};
